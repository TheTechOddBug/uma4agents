...
`draft-ietf-oauth-rar-metadata-remediation`'s parameters, carrying the same base64url
...
| 10 | Consumption ordering normative: introspect (non-consuming) → permissions → PoP → operation binding → `POST /consume`, atomic and last. Inactive introspection carries an `error`; `connection_revoked` is terminal | Unspecified where a single-use token is spent | Core §8.2–8.4, FedAuthz §6 |
| 11 | Requesting-agent descriptive metadata: CIMD `client_id` (resolved, self-reference enforced, display only) and Web Bot Auth `Signature-Agent` covered by the signature | The agent is its key, or its issuer's token | Core §5.2 |
| 12 | Challenge carries `error="insufficient_authorization"` + `authorization_remediation` (RFC 9396 `authorization_details` + `authorization_reference`), plus `authorization_server` and `ticket` inside it | UMA's challenge carries `as_uri` + `ticket` only | Core §3.3 |
